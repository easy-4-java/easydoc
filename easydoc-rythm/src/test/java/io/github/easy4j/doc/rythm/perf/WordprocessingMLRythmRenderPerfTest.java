 ...
 * the License.
 */
package io.github.easy4j.doc.rythm.perf;
...
import io.github.easy4j.doc.rythm.WordprocessingMLRythmTemplate;

/**
 * Timing regression for the Rythm engine render step and the end-to-end
 ...
 * 1 000 measured renders of the existing {@code /tpl/hello.tpl} template; the
 * median per call is asserted against a bound roughly 5x+ observed.</p>
 *
 * <p>Measured baseline (JDK 21, Apple Silicon, surefire):
 *   - render /tpl/hello.tpl: ~40 us per call (median over 1 000 renders)
 *   - process /tpl/hello.tpl (end-to-end docx): ~11 ms per call (median of 3)</p>
 */
class WordprocessingMLRythmRenderPerfTest {

    private static final Logger LOG = LoggerFactory.getLogger(WordprocessingMLRythmRenderPerfTest.class);

    private static final String TEMPLATE = "/tpl/hello.tpl";
    ...
    private static class ExposedTemplate extends WordprocessingMLRythmTemplate {
        @Override
    ...
    void renderHelloTemplateUnderBound() throws Exception {
        ...
        LOG.debug("rythm render {}: {} us median over {} renders", TEMPLATE, medianMicros, RENDER_MEASURED);
        assertTrue(out.contains("Hello world"));
        assertTrue(medianMicros < MAX_MEDIAN_RENDER_MICROS,
                "rythm render median " + medianMicros + " us exceeded bound " + MAX_MEDIAN_RENDER_MICROS + " us");
    }
    ...
    void processHelloTemplateEndToEndUnderBound() throws Exception {
        ...
        LOG.debug("rythm process {} end-to-end: {} ms median over {} renders", TEMPLATE, medianMillis, PROCESS_MEASURED);
        assertTrue(medianMillis < MAX_MEDIAN_PROCESS_MILLIS,
                "rythm process median " + medianMillis + " ms exceeded bound "
