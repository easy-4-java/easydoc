 ...
 * the License.
 */
package io.github.easy4j.doc.freemarker.perf;
...
import io.github.easy4j.doc.freemarker.WordprocessingMLFreemarkerTemplate;

/**
 * Timing regression for the Freemarker engine render step and the end-to-end
 * template adapter path ({@code process}).
 *
 * <p>Mirrors {@code WordprocessingMLFreemarkerHelloTest}: the engine needs a
 * {@link FileTemplateLoader} for the test {@code /tpl} directory, the template
 * name is {@code hello.ftl}, and the variables map must be mutable because
 * {@code render()} puts internal keys into it.</p>
 *
 ...
 *   - render hello.ftl: ~6 us per call (median over 1 000 renders)
 *   - process hello.ftl (end-to-end docx): ~10 ms per call (median of 3)</p>
 */
class WordprocessingMLFreemarkerRenderPerfTest {

    private static final Logger LOG = LoggerFactory.getLogger(WordprocessingMLFreemarkerRenderPerfTest.class);

    private static final String TEMPLATE = "hello.ftl";
    ...
    private static File tplDir;

    @BeforeAll
    static void setUpTemplateDir() {
        tplDir = new File(WordprocessingMLFreemarkerRenderPerfTest.class.getResource("/tpl/").getPath());
    }

    /** Local subclass exposing the protected engine render step for direct timing. */
    private static class ExposedTemplate extends WordprocessingMLFreemarkerTemplate {
        @Override
    ...
    private static <T extends WordprocessingMLFreemarkerTemplate> T configure(T t) throws Exception {
        t.setPreTemplateLoaders(new FileTemplateLoader(tplDir));
        return t;
    }
    ...
    void renderHelloTemplateUnderBound() throws Exception {
        ExposedTemplate t = configure(new ExposedTemplate());
        ...
        LOG.debug("freemarker render {}: {} us median over {} renders", TEMPLATE, medianMicros, RENDER_MEASURED);
        assertTrue(out.contains("Hello world"));
        assertTrue(medianMicros < MAX_MEDIAN_RENDER_MICROS,
                "freemarker render median " + medianMicros + " us exceeded bound " + MAX_MEDIAN_RENDER_MICROS + " us");
    }

    @Test
    @Timeout(value = 120)
    void processHelloTemplateEndToEndUnderBound() throws Exception {
        WordprocessingMLFreemarkerTemplate t = configure(new WordprocessingMLFreemarkerTemplate());
        Map<String, Object> vars = new HashMap<>(Map.of("name", "world"));
        ...
        LOG.debug("freemarker process {} end-to-end: {} ms median over {} renders", TEMPLATE, medianMillis, PROCESS_MEASURED);
        assertTrue(medianMillis < MAX_MEDIAN_PROCESS_MILLIS,
                "freemarker process median " + medianMillis + " ms exceeded bound "
