 ...
 * the License.
 */
package io.github.easy4j.doc.webit.perf;
...
import io.github.easy4j.doc.webit.WordprocessingMLWebitTemplate;

/**
 * Timing regression for the Webit engine render step and the end-to-end
 * template adapter path ({@code process}).
 *
 * <p>Mirrors {@code WordprocessingMLWebitHelloTest}: webit-script defaults to
 * {@code looseVar=false} which rejects undeclared variables, so both tests
 * enable {@code docx4j.webit.engine.looseVar} for the duration of the class
 ...
 *   - render /tpl/hello.html: ~2 us per call (median over 1 000 renders)
 *   - process /tpl/hello.html (end-to-end docx): ~10 ms per call (median of 3)</p>
 */
class WordprocessingMLWebitRenderPerfTest {

    private static final Logger LOG = LoggerFactory.getLogger(WordprocessingMLWebitRenderPerfTest.class);
    ...
    private static String prevLooseVar;

    @BeforeAll
    static void enableLooseVar() {
        ...
        prevLooseVar = props.getProperty("docx4j.webit.engine.looseVar");
        props.setProperty("docx4j.webit.engine.looseVar", "true");
    }

    @AfterAll
    static void restoreLooseVar() {
        Properties props = Docx4jProperties.getProperties();
        props.remove("docx4j.webit.engine.looseVar");
        if (prevLooseVar != null) {
            props.setProperty("docx4j.webit.engine.looseVar", prevLooseVar);
        }
    }

    /** Local subclass exposing the protected engine render step for direct timing. */
    private static class ExposedTemplate extends WordprocessingMLWebitTemplate {
        @Override
    ...
    void renderHelloTemplateUnderBound() throws Exception {
        ...
        LOG.debug("webit render {}: {} us median over {} renders", TEMPLATE, medianMicros, RENDER_MEASURED);
        assertTrue(out.contains("Hello world"));
        assertTrue(medianMicros < MAX_MEDIAN_RENDER_MICROS,
                "webit render median " + medianMicros + " us exceeded bound " + MAX_MEDIAN_RENDER_MICROS + " us");
    }
    ...
    void processHelloTemplateEndToEndUnderBound() throws Exception {
        ...
        LOG.debug("webit process {} end-to-end: {} ms median over {} renders", TEMPLATE, medianMillis, PROCESS_MEASURED);
        assertTrue(medianMillis < MAX_MEDIAN_PROCESS_MILLIS,
                "webit process median " + medianMillis + " ms exceeded bound "
