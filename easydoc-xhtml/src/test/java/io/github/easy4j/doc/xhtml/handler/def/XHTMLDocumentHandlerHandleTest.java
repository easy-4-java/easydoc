 ...
 * that are not exercised by the higher-level builder tests: File, InputStream,
 * and URL local protocol (file:).
 */
class XHTMLDocumentHandlerHandleTest {
    ...
    // handle(URL) with file: protocol -- exercises the local protocol branch
    @Test
    void handleFileUrlReturnsNonNullDocument() throws Exception {
        DocumentHandler handler = XHTMLDocumentHandler.getDocumentHandler();
        URL fileUrl = reportHtml().toURI().toURL();
        Document doc = handler.handle(fileUrl);
        assertNotNull(doc, "handle(URL) with file: protocol must return non-null");
        assertTrue(doc.html().length() > 0, "parsed document must have content");
    }
}
