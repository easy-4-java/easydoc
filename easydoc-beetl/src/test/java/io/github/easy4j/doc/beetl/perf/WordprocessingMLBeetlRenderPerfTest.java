 ...
 * the License.
 */
package io.github.easy4j.doc.beetl.perf;
...
import io.github.easy4j.doc.beetl.WordprocessingMLBeetlTemplate;

/**
 * Timing regression for the Beetl engine render step and the end-to-end
 * template adapter path ({@code process}) — the pipeline the Beetl delimiter
 * configuration fix (review) rides on.
 *
 ...
 * 1 000 measured renders of the existing {@code /tpl/hello.btl} template; the
 * median per call is asserted against a bound roughly 5x+ observed.</p>
 *
 * <p>Measured baseline (JDK 21, Apple Silicon, surefire):
 *   - render /tpl/hello.btl: ~8 us per call (median over 1 000 renders)
 *   - process /tpl/hello.btl (end-to-end docx): ~10 ms per call (median of 3)</p>
 */
class WordprocessingMLBeetlRenderPerfTest {

    private static final Logger LOG = LoggerFactory.getLogger(WordprocessingMLBeetlRenderPerfTest.class);

    private static final String TEMPLATE = "/tpl/hello.btl";
    ...
    private static class ExposedTemplate extends WordprocessingMLBeetlTemplate {
        @Override
    ...
    void renderHelloTemplateUnderBound() throws Exception {
        ...
        LOG.debug("beetl render {}: {} us median over {} renders", TEMPLATE, medianMicros, RENDER_MEASURED);
        assertTrue(out.contains("Hello world"));
        assertTrue(medianMicros < MAX_MEDIAN_RENDER_MICROS,
                "beetl render median " + medianMicros + " us exceeded bound " + MAX_MEDIAN_RENDER_MICROS + " us");
    }
    ...
    void processHelloTemplateEndToEndUnderBound() throws Exception {
        ...
        LOG.debug("beetl process {} end-to-end: {} ms median over {} renders", TEMPLATE, medianMillis, PROCESS_MEASURED);
        assertTrue(medianMillis < MAX_MEDIAN_PROCESS_MILLIS,
                "beetl process median " + medianMillis + " ms exceeded bound " + MAX_MEDIAN_PROCESS_MILLIS + " ms");
    }
