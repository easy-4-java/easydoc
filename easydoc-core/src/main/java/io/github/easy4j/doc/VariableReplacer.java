...
public interface VariableReplacer {
	 ...
	 *
	 * <p><b>工厂层说明：</b>{@link DocxTemplates#create(DocxMode)} 在 JDK 21+
	 * 上已经把 {@code DocxMode.SAX} 短路为
	 * {@code WordprocessingMLDocxStAXTemplate}（INFO 日志只打一次）。
	 * 此处的运行时降级只在调用方绕过工厂、直接
	 * {@code new WordprocessingMLDocxSaxTemplate()} 时才生效。
	 */
	final class Sax implements VariableReplacer {
		// volatile + 单次触发：beforeProcess() 可能被多个虚拟线程并发调用；
		// 该标志必须安全发布且只被置位一次。
