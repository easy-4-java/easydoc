 ...
 * the License.
 */
package io.github.easy4j.doc.httl.perf;
...
import io.github.easy4j.doc.httl.WordprocessingMLHttlTemplate;

/**
 * Timing regression for the HTTL engine render step and the end-to-end
 ...
 * 1 000 measured renders of the existing {@code /tpl/hello.htt} template; the
 * median per call is asserted against a bound roughly 5x+ observed.</p>
 *
 * <p>Measured baseline (JDK 21, Apple Silicon, surefire):
 *   - render /tpl/hello.htt: ~3 us per call (median over 1 000 renders)
 *   - process /tpl/hello.htt (end-to-end docx): ~11 ms per call (median of 3)</p>
 */
class WordprocessingMLHttlRenderPerfTest {

    private static final Logger LOG = LoggerFactory.getLogger(WordprocessingMLHttlRenderPerfTest.class);

    private static final String TEMPLATE = "/tpl/hello.htt";
    ...
    private static class ExposedTemplate extends WordprocessingMLHttlTemplate {
        @Override
    ...
    void renderHelloTemplateUnderBound() throws Exception {
        ...
        LOG.debug("httl render {}: {} us median over {} renders", TEMPLATE, medianMicros, RENDER_MEASURED);
        assertTrue(out.contains("Hello world"));
        assertTrue(medianMicros < MAX_MEDIAN_RENDER_MICROS,
                "httl render median " + medianMicros + " us exceeded bound " + MAX_MEDIAN_RENDER_MICROS + " us");
    }
    ...
    void processHelloTemplateEndToEndUnderBound() throws Exception {
        ...
        LOG.debug("httl process {} end-to-end: {} ms median over {} renders", TEMPLATE, medianMillis, PROCESS_MEASURED);
        assertTrue(medianMillis < MAX_MEDIAN_PROCESS_MILLIS,
                "httl process median " + medianMillis + " ms exceeded bound " + MAX_MEDIAN_PROCESS_MILLIS + " ms");
    }
